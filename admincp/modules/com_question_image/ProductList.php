<?php
    if (($_GET['action'] == 'del') && isset($_GET['id'])) {
        $id = intval($_GET['id']);
        delete_question_image($id);
        if ($_GET['order'] != "") {
            header('location:./?show=questionimage&order=' . $_GET['order'] . '&p=' . intval($_GET['p']));
        } else {
    ...
    $title = 'Danh sách câu hỏi ảnh luận';
    ...
    $content = $xtemplate->replace($content, array(
        ...
        'defaultText' => 'Tìm câu hỏi ảnh luận: ',
