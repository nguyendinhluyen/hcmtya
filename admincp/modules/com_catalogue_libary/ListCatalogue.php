<?php
    ...
    $mysql->setQuery("SELECT categorieslibary_id
                    ...
                    WHERE languagelibary ='" . $_SESSION['lag'] . "'"); // and level<>1");
    $mysql->query();
    ...
    $sql = "SELECT * 
            FROM categorieslibary 
            WHERE languagelibary ='" . $_SESSION['lag'] . "' order by " . $orderby . " limit " . $limitvalue . "," . $pp;
    $mysql->setQuery($sql);
    ...
    $content = $xtemplate->replace($content, array(
        'lag_cat_manager' => "Danh mục tin tức sự kiện",
        'lag_page' => $arr_lang['page'],
        'page' => page_div("./?show=catalogue_libary&p=%d_pg&order=" . $_GET['order'], "10", ceil($numofpages), $page),
