<?php
    ...
    if ($_SESSION['timeout'] + 0 < time()) {
        $title = '&nbsp;Bài viết kế hoạch cuộc thi';
        $id = intval($_GET['id']);
        $btn = $arr_lang['add'];
        $panel = $title;
        $update = 0;
        if (check_id_news_introduce($id)) { //fill data on textbox : check_valid($id)
            $update = 1;
            $btn = $arr_lang['update'];
            $panel = '&nbsp;Bài viết kế hoạch cuộc thi';
            $title = $panel;
        ...
        if (isset($_POST['btnadd'])) {
            ...
            if (!isset($news_title{4}))
                $error.= '<li>Tiêu đề quá ngắn</li>';

            if (!isset($categories_id{0}))
                $error.= '<li>Bạn chưa chọn danh mục cho bài viết</li>';

            if (!isset($news_key{0})) {
                $error.= '<li> Chưa có tiêu đề</li>';
            }
            ...
            if (CheckTonTaiBaiViet($news_key) == true && $update != 1) {
                $error.= '<li> Tiêu đề đã tồn tại, vui lòng chọn tên khác </li>';
            }
        ...
        $content = $xtemplate->replace($content, array(
            'error' => $error,
            'help_list' => catlist_introduce($categories_id), 
            'panel_add_update_product' => $panel,
            'news_key' => 'Link',
            'news_title' => 'Tiêu đề',
            'news_detail' => 'Bài viết kế hoạch',
            'btn' => $btn,
            'txtname' => $news_title,
            'txtdetail' => $detail,
