...
function check_cats_name_result($catname, $id, $parent_id) {
    return checkExit('categoriesresult_id', 'categoriesresult', "categoriesresult_name = '" . $catname
            . "' AND languageresult ='" . $_SESSION['lag']
            . "' AND categoriesresult_id <>" . $id);
}
...
function check_valid_result($id) {
    return checkExit("categoriesresult_id"
            , "categoriesresult"
            , "categoriesresult_id = '" . $id
            . "' AND languageresult ='" . $_SESSION['lag'] . "'");
}
...
function check_id_question($id) {
    return checkExit('question_id', 'question', "question_id  = '" . $id
            . "' and language ='" . $_SESSION['lag']
            . "'");
}

function check_id_question_image($id) {
    return checkExit('question_id', 'question_image', "question_id  = '" . $id
            . "' and language ='" . $_SESSION['lag']
            . "'");
}
...
function check_id_news_result($id) {
    return checkExit('newsresult_id', 'news_result', "newsresult_id  = '" . $id
            . "' and languageresult ='" . $_SESSION['lag'] . "'");
}
...
function getLevel_result($id) {
    global $mysql;
    $mysql->setQuery("SELECT levelresult 
                    FROM categoriesresult 
                    WHERE categoriesresult_id ='" . $id . "'");
    $mysql->query();
    $row = $mysql->loadOneRow();
    return $row['levelresult'];
}
...
function catlist_introduce($id) {
    ...
    $sql = "SELECT categoriesintroduce_id,
            ...
            FROM categoriesintroduce 
            WHERE levelintroduce = 1 AND languageintroduce = '" . $_SESSION['lag'] . "'";
    $mysql->setQuery($sql);
    $result = $mysql->query();
    $select = "<select name = 'catlist' id = 'catlist'>";
    while ($row = mysql_fetch_assoc($result)) {
        $sql2 = "SELECT categoriesintroduce_id,
                        categoriesintroduce_name
                FROM categoriesintroduce 
...
function catlist_result($id) {
    global $mysql;
    $sql = "SELECT categoriesresult_id,
                   categoriesresult_name
            FROM categoriesresult 
            WHERE levelresult = 1 AND languageresult = '" . $_SESSION['lag'] . "'";
    $mysql->setQuery($sql);
    $result = $mysql->query();
    $select = "<select name = 'catlist' id = 'catlist'>";
    while ($row = mysql_fetch_assoc($result)) {
        $sql2 = "SELECT categoriesresult_id,
                        categoriesresult_name
                FROM categoriesresult 
                WHERE levelresult = 2
                    AND parentresult_id ='" . $row['categoriesresult_id'] . "'
                    AND languageresult = '" . $_SESSION['lag'] . "'";
        $mysql->setQuery($sql2);
        $result2 = $mysql->query();

        if (mysql_num_rows($result2) > 0) {
            $select .= "<optgroup label='" . $row['categoriesresult_name'] . "'>";

            while ($row2 = mysql_fetch_assoc($result2)) {
                if ($id == $row2['categoriesresult_id'])
                    $select .= "<option value ='" . $row2['categoriesresult_id']
                            . "' selected>" . $row2['categoriesresult_name']
                            . "</option>";
                else
                    $select .= "<option value ='" . $row2['categoriesresult_id']
                            . "'>" . $row2['categoriesresult_name'] . "</option>";
            }

            $select .= "</optgroup>";
        }
        else {
            if ($id == $row['categoriesresult_id'])
                $select .= "<option value ='" . $row['categoriesresult_id']
                        . "' selected>" . $row['categoriesresult_name'] . "</option>";
            else
                $select .= "<option value ='" . $row['categoriesresult_id'] . "'>"
                        . $row['categoriesresult_name'] . "</option>";
        }
    }
    $select .= '</select>';
    return $select;
...
function dl_subcats_introduce($id = '', $width = 100) {
    global $mysql;
    $sql = "SELECT categoriesintroduce_id,categoriesintroduce_name
            FROM categoriesintroduce
            WHERE levelintroduce = 1 and languageintroduce = '" . $_SESSION['lag'] . "'";
    $mysql->setQuery($sql);
    $result = $mysql->query();
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        $sql2 = "SELECT categoriesintroduce_id,categoriesintroduce_name 
                 FROM categoriesintroduce
                 WHERE levelintroduce = 2 and parentintroduce_id ='"
...
function dl_subcats_result($id = '', $width = 100) {
    global $mysql;
    $sql = "SELECT categoriesresult_id,categoriesresult_name
            FROM categoriesresult
            WHERE levelresult = 1 and languageresult = '" . $_SESSION['lag'] . "'";
    $mysql->setQuery($sql);
    $result = $mysql->query();
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        $sql2 = "SELECT categoriesresult_id,categoriesresult_name 
                 FROM categoriesresult
                 WHERE levelresult = 2 and parentresult_id ='"
                . $row['categoriesresult_id'] . "' and languageresult = '"
                . $_SESSION['lag'] . "'";
...
function get_parent_name_result($id) {
    global $mysql;
    $sql = "SELECT categoriesresult_name 
            FROM categoriesresult 
            WHERE categoriesresult_id = " . intval($id);
    $mysql->setQuery($sql);
    $mysql->query();
    if ($mysql->getNumRows() > 0) {
        $row = $mysql->loadOneRow();
        return $row['categoriesresult_name'];
    } else {
        return 0;
    }
}
...
function get_cats_name_result($id) {
    global $mysql;
    $sql = "SELECT categoriesresult_name	 
            FROM categoriesresult
            WHERE categoriesresult_id = '" . intval($id) . "'";
    ...
    $mysql->query();
    $arr = $mysql->loadOneRow();
    return $arr['categoriesresult_name'];
}
...
// Delete question
function delete_question($arr_id) {
    global $mysql;
    if (!empty($arr_id)) {
        $sql = "DELETE FROM question WHERE question_id IN (" . $arr_id . ")";
        $mysql->setQuery($sql);
        $mysql->query();
    }
}

function delete_question_image($arr_id) {
    global $mysql;
    if (!empty($arr_id)) {
        $sql = "DELETE FROM question_image WHERE question_id IN (" . $arr_id . ")";
        $mysql->setQuery($sql);
        $mysql->query();
    }
}
...
function delete_news_result($arr_id) {
    global $mysql;
    if (!empty($arr_id)) {
        $sql = "DELETE FROM news_result WHERE newsresult_id in (" . $arr_id . ")";
        $mysql->setQuery($sql);
        $mysql->query();
    }
}
...
function Set_status_news_result($arr_id) {
    SetStatus('newsresult_id'
            , 'statusresult'
            , 'news_result'
            , 'newsresult_id in (' . $arr_id . ')');
}
...
function Set_status_catalogue_introduce($arr_id) {
    SetStatus('categoriesintroduce_id'
            , 'categoriesintroduce_status'
            , 'categoriesintroduce'
            , 'categoriesintroduce_id in (' . $arr_id . ')');
}

function Set_status_catalogue_result($arr_id) {
    SetStatus('categoriesresult_id'
            , 'categoriesresult_status'
            , 'categoriesresult'
            , 'categoriesresult_id in (' . $arr_id . ')');
}
...
function Set_status_question($arr_id) {
    SetStatus('question_id', 'question_status', 'question', 'question_id in (' . $arr_id . ')');
}

function Set_status_question_image($arr_id) {
    SetStatus('question_id', 'question_status', 'question_image', 'question_id in (' . $arr_id . ')');
}
...
function delete_cats_result($arr_id) {
    global $mysql;
    $sql = 'DELETE 
            FROM categoriesresult 
            WHERE categoriesresult_id in (' . $arr_id . ')';
    $mysql->setQuery($sql);
    $mysql->query();
}
