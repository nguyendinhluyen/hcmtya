...
class News {
    ...
    public static function getDetailofResultByCategory($newskey) {
        $category = GetOneRow('categoriesresult_id'
                , 'categoriesresult'
                , 'categoriesresult_key = "' . $newskey . '"');
        $news_result = GetOneRow('newsresult_key'
                , 'news_result'
                , 'newsresult_catalogue = ' . $category['categoriesresult_id']);
        return $news_result['newsresult_key'];
    }
    ...
    public static function getFistRowsOfResultByCategory() {
        $category = GetRows('categoriesresult_id'
                , 'categoriesresult'
                , 'levelresult = 1 ORDER BY sortresult_order ASC');
        if (!empty($category)) {
            $news_result = GetOneRow('newsresult_key'
                    , 'news_result'
                    , 'newsresult_catalogue = ' . $category[0]['categoriesresult_id']);
            if (!empty($news_result)) {
                return $news_result['newsresult_key'];
            } else if (empty($news_result)) {
                $news_result_child = GetRows('categoriesresult_id'
                        , 'categoriesresult'
                        , 'levelresult = 2 AND parentresult_id = '
                        . $category[0]['categoriesresult_id'] . ' ORDER BY sortresult_order ASC');
                $news_result = GetOneRow('newsresult_key'
                        , 'news_result'
                        , 'newsresult_catalogue = ' . $news_result_child[0]['categoriesresult_id']);
                return $news_result['newsresult_key'];
            }
        }
        return -1;
    ...
    public static function getDetailNews_Result($news_key) {
        $info = GetOneRow('newsresult_name,
                        newsresult_content,
                        newsresult_key,						  
                        dateresult_added'
                , 'news_result'
                , "newsresult_key = '" . $news_key . "'");
        return $info;
    }
