<?php
    class Product {
        public static function getQuestion($question_id) {
            $question = GetOneRow("question_id"
                                . ", question_detail"
                                , "question"
        ...
        public static function getQuestionImage($question_id) {
            $question = GetOneRow("question_id"
                                . ",question_detail"
                                . ",question_image"
                                , "question_image"
        ...
        public static function calculateScore($selected_id) {
            $score = 0;
            $multchoice = explode(";", $selected_id);
            for ($i = 1; $i < count($multchoice) - 1; $i++) {
                $question_answer = explode("_", $multchoice[$i]);
                if (!empty($question_answer[0]) && !empty($question_answer[1])) {
                    $mult_choices = GetOneRow("products_id"
                                            . ", result"
                                            , "products"
                                            , "products_id = " . $question_answer[0]);
                    if (intval($mult_choices['result']) === intval($question_answer[1])) {
                        $score ++;
                    }
                }
            }
        ...
        public static function userFullInfo($username) {
            $userInfo = GetOneRow("name"
                                . ", avarta"
                                . ", phone"
                                . ", address"
                                . ", email"
                            , "user"
                            , "email = '" . $username . "'");
            if (empty(trim($userInfo['email']))
                    || empty (trim($userInfo['name']))
                    || empty (trim($userInfo['avarta']))
                    || empty (trim($userInfo['phone']))
                    || empty (trim($userInfo['address']))) {
                return 0;
            }
            return 1;
        }
        
        public static function timeFinish($username) {
            $examFinish = GetOneRow("name"
                    . ", birthday"
                    . ", start_time_exam"
                    . ", end_time_exam"
                    . ", start_time_exam_question"
                    . ", end_time_exam_question"
                    . ", start_time_exam_question_image"
                    . ", end_time_exam_question_image"
                    . ", exam_question"
                    . ", exam_question_image"
                    , "user"
                    , "email = '" . $username . "'");
        ...
        public static function isUserFinishExam($username) {
            $examFinish = GetRows("id, exam_check"
                                , "user"
                                , "email = '" . $username . "' AND exam IS NOT NULL");
            return $examFinish[0]["exam_check"];
        }
        
        public static function isUserFinishExamQuestion($username) {
            $examFinish = GetRows("id, exam_check_question"
                                , "user"
                                , "email = '" . $username . "'");
            return $examFinish[0]["exam_check_question"];
        }
        
        public static function isUserFinishExamQuestionImage($username) {
            $examFinish = GetRows("id, exam_check_question_image"
                                , "user"
                                , "email = '" . $username . "'");
            return $examFinish[0]["exam_check_question_image"];
        }
        
        public static function isUserFinishMultiChoiceExam($username) {
            $examFinish = GetRows("id, exam_check"
                                , "user"
                                , "email = '" . $username . "'");
            return $examFinish[0]["exam_check"];
        ...
        public static function isUserExam($username) {
            $examCategory = GetRows("id,exam"
                                , "user"
                                , "email = '" . $username . "' AND exam IS NOT NULL");
            return $examCategory[0]["exam"];
        }
        
        public static function isUserExamQuestion($username) {
            $examQuestion = GetRows("id, exam_question"
                                , "user"
                                , "email = '" . $username . "' AND exam_question IS NOT NULL");
            return $examQuestion[0]["exam_question"];
        }
        
        public static function isUserExamQuestionImage($username) {
            $examQuestion = GetRows("id, exam_question_image"
                                , "user"
                                , "email = '" . $username . "' AND exam_question_image IS NOT NULL");
            return $examQuestion[0]["exam_question_image"];
        }

        public static function getListAllMutilChoiceCategory() {
            $examCategory = GetRows('categories_id'
                    , 'categories'
                    , "categories_status = 1 ORDER BY date_added ASC");
            return $examCategory;
        }
        
        public static function getListAllQuestion() {
            $examQuestion = GetRows('question_id'
                                , 'question'
                                , "question_status = 1 ORDER BY question_date_added ASC");
            return $examQuestion;
        }
        
        public static function getListAllQuestionImage() {
            $examQuestion = GetRows('question_id'
                                , 'question_image'
                                , "question_status = 1 ORDER BY question_date_added ASC");
            return $examQuestion;
        ...
        public static function getListAllMutilChoice($category) {
            $introduce = GetRows('products_id'
                    . ', products_name'
                    . ', resultfirst'
                    . ', resultsecond'
                    . ', resultthird'
                    , 'products'
                    , "products_status = 1 "
                    . 'AND CONCAT(" ",upgrade_categories_id," ") LIKE "%,' . $category . ',%" '
