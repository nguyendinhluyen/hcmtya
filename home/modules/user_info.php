<?php
    ...
    if (isset($_SESSION['username'])) {
        ...
        if (isset($_GET['type'])) {
            ...
            if ($type == 'update') {
                ...
                if (empty(trim($number_address))
                        ...
                        || empty(trim($name))
                        || empty(trim($phone))
                        || (empty(trim($image)) && empty($row_ac['avarta']))) {
                    ?>				
                    <script>
                        alert('Vui lòng cung cấp họ tên, địa chỉ, số điện thoại và ảnh đại diện (nếu chưa có)!');
        ...
        if (empty($row_ac['avarta'])) {
            $message_notice .= " và ảnh đại diện";
        }
        
        if ($message_notice != "Hãy cập nhật") {
            $message_notice .= " để tham gia thi bạn nhé!";
    ...
    $breadcrumbs_path .= 'Thông Tin Tài Khoản';
    $tilte_page = 'Thông Tin Tài Khoản | ' . 'Sở du lịch Hồ Chí Minh';
    $content = $user_info;
?>
